...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def _optimizer_step(self, sub_group_id):
        ...
        len = self.fp32_partitioned_groups_flat[sub_group_id].numel()
        self.fp32_param[:len].copy_(self.fp32_partitioned_groups_flat[sub_group_id])
        self.fp32_param.grad[:len].copy_(self.fp32_partitioned_groups_flat[sub_group_id].grad)
        ...
        if self.offload_optimizer:
            ...
            #else:
            #print("gpu \n")
            self.backup_optimizer.param_groups[param_group_id]['params'] = [self.fp32_param]
            step_with_gradscaler(self.backup_optimizer)
            self.backup_optimizer.param_groups[param_group_id]['params'] = []
            self.fp32_partitioned_groups_flat[sub_group_id].grad.copy_(self.fp32_param.grad[:len])
            self.fp32_partitioned_groups_flat[sub_group_id].copy_(self.fp32_param[:len])
        else:
            self.optimizer.param_groups[param_group_id]['params'] = [fp32_param]
            step_with_gradscaler(self.optimizer)
            self.optimizer.param_groups[param_group_id]['params'] = []
    ...
    def initialize_optimizer_states(self):
        ...
        self.fp32_param = torch.zeros(int(largest_numel), dtype=self.master_weights_and_grads_dtype, device=get_accelerator().current_device_name())

        self.fp32_param.grad = torch.zeros(int(largest_numel), dtype=gradient_dtype, device=get_accelerator().current_device_name()) 
