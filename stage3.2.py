...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def _optimizer_step(self, sub_group_id, pre):
        param_group_id = self.sub_group_to_group_id[sub_group_id]
        fp32_param = self.fp32_partitioned_groups_flat[sub_group_id]
 
        #self.device = get_accelerator().current_device_name
        #self.fp32_param = self.fp32_partitioned_groups_flat[sub_group_id].to(get_accelerator().current_device_name())
        #self.fp32_param.grad = self.fp32_partitioned_groups_flat[sub_group_id].grad.to(get_accelerator().current_device_name())

        if pre == True:
             lenh = self.fp32_partitioned_groups_flat[sub_group_id+1].numel()
             if sub_group_id % 2:
                 self.fp32_param_ping[:lenh].copy_(self.fp32_partitioned_groups_flat[sub_group_id+1], non_blocking=True)
                 self.fp32_param_ping.grad[:lenh].copy_(self.fp32_partitioned_groups_flat[sub_group_id+1].grad, non_blocking=True)
             else:
                 self.fp32_param_pong[:lenh].copy_(self.fp32_partitioned_groups_flat[sub_group_id+1], non_blocking=True)
                 self.fp32_param_pong.grad[:lenh].copy_(self.fp32_partitioned_groups_flat[sub_group_id+1].grad, non_blocking=True)
        ...
        if self.offload_optimizer:
            ...
            #else:
            #print("gpu \n")
            lenh = self.fp32_partitioned_groups_flat[sub_group_id].numel()
            if sub_group_id % 2:
                 self.backup_optimizer.param_groups[param_group_id]['params'] = [self.fp32_param_pong]
                 step_with_gradscaler(self.backup_optimizer)
                 self.backup_optimizer.param_groups[param_group_id]['params'] = []

                 self.fp32_partitioned_groups_flat[sub_group_id].grad.copy_(self.fp32_param_pong.grad[:lenh])
                 self.fp32_partitioned_groups_flat[sub_group_id].copy_(self.fp32_param_pong[:lenh])

            else:
                 self.backup_optimizer.param_groups[param_group_id]['params'] = [self.fp32_param_ping]
                 step_with_gradscaler(self.backup_optimizer)
                 self.backup_optimizer.param_groups[param_group_id]['params'] = []

                 self.fp32_partitioned_groups_flat[sub_group_id].grad.copy_(self.fp32_param_ping.grad[:lenh])
                 self.fp32_partitioned_groups_flat[sub_group_id].copy_(self.fp32_param_ping[:lenh])
    ...
    def initialize_optimizer_states(self):
        ...
        self.fp32_param_ping = torch.zeros(int(largest_numel), dtype=self.master_weights_and_grads_dtype, device=get_accelerator().current_device_name())
        self.fp32_param_ping.grad = torch.zeros(int(largest_numel), dtype=gradient_dtype, device=get_accelerator().current_device_name()) 

        self.fp32_param_pong = torch.zeros(int(largest_numel), dtype=self.master_weights_and_grads_dtype, device=get_accelerator().current_device_name())
        self.fp32_param_pong.grad = torch.zeros(int(largest_numel), dtype=gradient_dtype, device=get_accelerator().current_device_name())
    ...
    def step(self, closure=None):
        ...
        for sub_group_id, group in enumerate(self.fp16_groups):
            ...
            if sub_group_id == 0:
                lenh = self.fp32_partitioned_groups_flat[sub_group_id].numel()
                self.fp32_param_ping[:lenh].copy_(self.fp32_partitioned_groups_flat[sub_group_id])
                self.fp32_param_ping.grad[:lenh].copy_(self.fp32_partitioned_groups_flat[sub_group_id].grad)

            if (sub_group_id != len(self.fp16_groups) - 1):
                pre = True
            else:
                pre = False

            self._optimizer_step(sub_group_id, pre)
